...
#include "ann/storage/tensor.hpp"
...
struct Clustering {
  ...
  using SymComputerType =
      SymComputerImpl<Tensor, helpa::l2_fp32_fp32, float, float>;
  using ComputerType =
      ComputerImpl<Tensor, helpa::l2_fp32_fp32, float, float, float, float>;

  constexpr static int32_t kSampleNum = 500000;
  ...
  Clustering(int32_t n_cluster, int32_t epochs = 20,
...
private:
  ...
  void init_kmeanspp(const Tensor &tensor) {
    ...
    while (num_picked < n_cluster) {
      ...
      if (std::find(idx.begin(), idx.end(), tmp_pivot) != idx.end() &&
          sum_flag == false) {
        continue;
      }
...
#pragma omp parallel for schedule(dynamic)
  ...
  find_index(const Tensor &tensor, const std::vector<float> &dists) const {
    double loss = 0.0;
    int32_t n = tensor.size();

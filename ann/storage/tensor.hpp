...
struct Tensor {
  ...
  Tensor(int32_t dim, int32_t nbits, int32_t align_width)
      : d(dim), dalign((dim + align_width - 1) / align_width * align_width),
        csize(dalign * nbits / 8) {}

  ~Tensor() { free(codes); }

  Tensor(int32_t n, int32_t dim, int32_t nbits, int32_t align_width)
      : Tensor(dim, nbits, align_width) {
    init(n);
  }
